...
class TestFeatureStore20KTPS:
    ...
    def entity_rows(self):
        ...
        return [{"account_id": f"acc-{i % 10000}", "timestamp": time.time()} for i in range(10000)]
    ...
    def test_feature_lookup_20k_tps_concurrent(self, feature_store, entity_rows):
        ...
        latencies = []
        errors = 0
        # Note: List append is thread-safe for CPython, no lock needed
        ...
        with ThreadPoolExecutor(max_workers=self.CONCURRENCY) as executor:
            ...
            for i in range(num_batches):
                batch = [
                    entity_rows[(i * batch_size + j) % len(entity_rows)] for j in range(batch_size)
                ]
...
class TestFeatureStoreBatchLookup:
    ...
    def test_batch_feature_lookup_performance(self, feature_store):
        ...
        for batch_size in batch_sizes:
            _entity_rows = [
                {"account_id": f"acc-{i}", "timestamp": time.time()} for i in range(batch_size)
            ]
        ...
        for i in range(1, len(results)):
            ...
            assert tps_ratio > 0.8, f"Batch efficiency degraded at size {results[i]['batch_size']}"
