...
consumer_lag_records = _SafeMetric(Gauge(
...
last_checkpoint_duration_ms = _SafeMetric(Gauge(
...
dedup_skipped_total = _SafeMetric(Counter(
    "dedup_skipped_total",
...
late_events_within_window_total = _SafeMetric(Counter(
    "late_events_within_window_total",
    "Events that arrived late but within the allowed lateness window",
    labelnames=["stage"],
))

late_events_beyond_window_total = _SafeMetric(Counter(
...
corrected_record_latency_ms = _SafeMetric(Histogram(
