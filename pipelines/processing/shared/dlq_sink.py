...
_PROCESSING_DLQ_TOPIC = "txn.processing.dlq"


class ProcessingDLQSink:
    """Class wrapper around build_dlq_record / serialise_dlq_record with a Kafka producer.

    Satisfies pipelines.shared.dlq_protocol.DLQSink via structural subtyping.
    Accepts source_partition=0 / source_offset=0 when partition/offset metadata
    is unavailable (e.g. called outside a Flink operator context).
    """

    def __init__(
        self,
        bootstrap_servers: str,
        topic: str = _PROCESSING_DLQ_TOPIC,
        schema_id: int = 0,
    ) -> None:
        ...
        self._producer = Producer(
            {
                ...
                "acks": 1,
                "linger.ms": 5,
                "client.id": f"processing-dlq-{socket.gethostname()}",
            }
        )
        self._topic = topic
    ...
    def send(
        ...
        *,
        ...
        value = serialise_dlq_record(record, self._schema_id)
