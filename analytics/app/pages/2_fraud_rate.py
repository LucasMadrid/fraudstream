"""Fraud rate historical page — queries Iceberg via DuckDB."""
...
with tab_trend:
    ...
    try:
        df = fraud_rate_daily(hours=days * 24)
...
with tab_channel:
    ...
    try:
        ch_df = fraud_rate_by_channel(hours=days * 24)
    except Exception as e:
        st.error(f"Query failed: {e}")
    else:
        if ch_df.empty:
...
with tab_raw:
    ...
    try:
        raw = fraud_rate_daily(hours=days * 24)
    except Exception as e:
        st.error(f"Query failed: {e}")
    else:
        if raw.empty:
            ...
            st.caption("⚠️ `avg_fraud_score` is approximate (computed per decision date).")
